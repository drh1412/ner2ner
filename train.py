...
#                             pin_memory=False,
#                             drop_last=True)
TrainingLoader=datalo(im_folder='imageunregsit')
# testdatalo=datalo('data2',batch_size=1,lengthofdata=100,train=False)
# Validation Set
...
# Network
network = UNet(in_nc=opt.n_channel+1,
...
for epoch in range(1, opt.n_epoch + 1):
    ...
    for iteration, clean in enumerate(TrainingLoader):
        ...
        if iteration==0:
            ...
            with torch.no_grad():
                ...
                noisy_denoised = network(noisy)
                tiff.imsave('./imageresult/{}.tif'.format(epoch), np.float32(noisy_denoised.cpu().detach().numpy()))
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        # np.random.seed(101)
        # with torch.no_grad():
        #     imgs = torch.zeros((500, 512, 512))
        #     data = torch.zeros((500, 512, 512))
        ...
        #         imgs[j, :, :] = img
        #         data[j, :, :] = img2[:,0:1,:,:]
        #     tiff.imsave('denoise.tif', np.float32((imgs).cpu().detach().numpy()))
        #     tiff.imsave('noise.tif', np.float32(abs(imgs- data).cpu().detach().numpy()))
        #     tiff.imsave('data.tif', np.float32((data).cpu().detach().numpy()))
